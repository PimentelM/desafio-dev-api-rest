...
describe('ContaController (e2e)', () => {
  ...
  afterAll(async () => {
    if (process.env.USE_IN_MEMORY_MONGO) await mongod.stop();
  ...
  describe('/saldo/:contaid', () => {
    it('Não deve ser possível obter saldo de conta inexistente', async () => {
      let response = await request(httpServer).get(
        '/api/banking/conta/saldo/112233445566112233445566',
      );
  ...
  describe(`/extrato/:contaId`, () => {
    it('Não deve ser possível obter extrato de conta inexistente', async () => {
      let response = await request(httpServer).get(
        '/api/banking/conta/extrato/112233445566112233445566',
      );
    ...
    it('Deve ser possível obter todo o extrato', async () => {
      ...
      let response = await request(httpServer).get(
        `/api/banking/conta/extrato/${conta._id}`,
      );

      expect(response.status).toBe(200);
      expect(sortBy(response.body, 'dataTransacao')).toMatchObject(
        transactions,
      );
    });
    ...
    it('Deve ser possível filtrar a obtenção do extrato', async () => {
      ...
      let response = await request(httpServer).get(
        `/api/banking/conta/extrato/${conta._id}?inicioPeriodo=2013-11-18T11:45:26.232Z`,
      );
      ...
      response = await request(httpServer).get(
        `/api/banking/conta/extrato/${conta._id}?inicioPeriodo=2013-11-18T11:45:26.232Z&fimPeriodo=2013-12-16T12:30:53.331Z`,
      );
  ...
  describe(`/criar`, () => {
    it(`Deve ser possível criar uma conta`, async () => {
      ...
      let response = await request(httpServer)
        .post('/api/banking/conta/criar')
        .send(criarContaInput);
      ...
      let contaNoBancoDeDados = (
        await contaModel.findOne({ _id: response.body._id })
      ).toObject();
    ...
    it(`Não deve ser possível criar mais de uma conta por pessoa`, async () => {
      ...
      let response1 = await request(httpServer)
        .post('/api/banking/conta/criar')
        .send(criarContaInput);

      expect(response1.status).toBe(201);

      let response2 = await request(httpServer)
        .post('/api/banking/conta/criar')
        .send(criarContaInput);
      ...
      expect(response2.body).toMatchObject({
        message: 'A pessoa já possui uma conta.',
      });
    });
  });
  ...
  describe(`/depositar-valor`, () => {
    it('Deve ser possível depositar valores na conta', async () => {
      ...
      let response = await request(httpServer)
        .post(`/api/banking/conta/depositar-valor`)
        .send({
          conta: contaId,
          valor: 1000,
        });
    ...
    it('Não deve ser possível depositar dinheiro em uma conta inativa', async () => {
      // Cria uma conta de testes.
      let contaId = (
        await contaModel.create(getMockConta({ saldo: 0, flagAtivo: false }))
      )._id;
    ...
    it('Deve ser possível sacar valores da conta', async () => {
      ...
      let contaId = (
        await contaModel.create(
          getMockConta({ saldo: 1000, limiteSaqueDiario: 500 }),
        )
      )._id;

      // Faz um saque de 200
      let response = await request(httpServer)
        .post(`/api/banking/conta/sacar-valor`)
        .send({
          conta: contaId,
          valor: 200,
        });
  ...
  describe(`/bloquear-conta`, () => {
    it('Deve ser possível bloquear a conta', async () => {
      // Cria uma conta de testes
      let contaId = (await contaModel.create(getMockConta({ flagAtivo: true })))
        ._id;

      // Solicita bloqueio
      let response = await request(httpServer)
        .post(`/api/banking/conta/bloquear`)
        .send({
          conta: contaId,
        });

      let { flagAtivo } = await contaModel
        .findOne({ _id: contaId })
        .select('flagAtivo');
