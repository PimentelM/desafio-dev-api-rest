...
import {
    ...
    DepositarValorDto,
    SacarValorDto,
    Conta, Message, Saldo, TransacaoExtrato, TransacaoResultado
...
export class ContaController {
    ...
    @Get(`/saldo/:contaId`)
    saldoConta(@Param(`contaId`, ParseObjectIdPipe) conta: string) : Promise<Saldo>{
    ...
    extratoConta(@Param(`contaId`, ParseObjectIdPipe) conta: string,
                 ...
                 @Query('fimPeriodo', ParseDateTimePipe) fimPeriodo?: Date) : Promise<TransacaoExtrato[]>{
    ...
    @Post(`/criar`)
    criarConta(@Body() {pessoa, limiteSaqueDiario, flagAtivo, tipoConta}: CriarContaDto) : Promise<Conta> {
        return this.contaService.criarConta(pessoa,limiteSaqueDiario,flagAtivo,tipoConta)
    }

    @Post(`/depositar-valor`)
    depositarValor(@Body() {conta, valor}: DepositarValorDto) : Promise<TransacaoResultado>{
        return this.contaService.depositarValor(conta,valor)
    }

    @Post(`/sacar-valor`)
    sacarValor(@Body() {conta, valor}: SacarValorDto)  : Promise<TransacaoResultado> {
